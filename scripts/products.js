// products.js - cart, wishlist, render, checkout via WhatsApp + Firestore hook
...
function updateCartCount(){ const c = JSON.parse(localStorage.getItem('BHET_cart')||'[]'); const el=document.getElementById('cartCount'); if(el) el.textContent = c.length; }
...
function checkoutWhatsApp(){ const cart = JSON.parse(localStorage.getItem('BHET_cart')||'[]'); if(!cart.length) return alert('Cart empty'); const products = getProducts(); let total=0; let text='Order from BHET:%0A'; cart.forEach(it=>{ const pr = products.find(p=>p.id===it.id); if(pr){ total += pr.price * it.qty; text += pr.title + ' x'+it.qty + ' - ₹' + (pr.price*it.qty) + '%0A'; }}); text += 'Total: ₹'+total; if(typeof sendOrderToFirestore === 'function'){ sendOrderToFirestore({items:cart,total:total,createdAt:Date.now()}); } const wa = 'https://wa.me/91' + '7768082083' + '?text=' + text; window.open(wa,'_blank'); }
document.addEventListener('DOMContentLoaded', function(){ if(!localStorage.getItem('BHET_products')){} renderProducts(); updateCartCount(); });